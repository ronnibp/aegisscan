...
LANG_RULES = {
    ".py": [
        ...
        _f("Open redirect via unvalidated request parameter", Severity.MEDIUM, "Open Redirect",
           "The redirect target comes straight from the request. An attacker can craft a link that bounces the victim to a phishing site using your domain's trust.",
           "Validate the destination against a strict allow-list of paths/hosts before redirecting.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html"], ["T1190"], "CWE-601", ["redirect"]),
        _f("Path traversal via user-controlled file path (send_file)", Severity.HIGH, "Path Traversal",
           "A file is served from a request-derived path; '../' sequences can read arbitrary files on the server.",
           "Map user input to a fixed allow-list of files, or normalize+prefix a base directory and reject '..'.",
           ["https://owasp.org/www-community/attacks/Path_Traversal"], ["T1190", "T1083", "T1005"], "CWE-22", ["traversal"]),
        _f("Path traversal via os.path.join with user input", Severity.MEDIUM, "Path Traversal",
           "os.path.join with an absolute or '../'-laden user value escapes the intended base directory.",
           "Normalize (os.path.realpath) and verify the result stays under the base directory; reject absolute input.",
           ["https://owasp.org/www-community/attacks/Path_Traversal"], ["T1190", "T1083"], "CWE-22", ["traversal"]),
        _f("JWT signature verification disabled", Severity.CRITICAL, "Authentication",
           "Decoding JWTs with verification disabled lets anyone forge tokens (admin, session, password reset).",
           "Always verify signatures: jwt.decode(token, key, algorithms=['HS256']) — never pass verify=False.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html"], ["T1078", "T1190"], "CWE-347", ["jwt", "auth"]),
        _f("JWT 'none' algorithm accepted", Severity.CRITICAL, "Authentication",
           "Accepting alg=none (or an attacker-chosen algorithm) allows unsigned token forgery.",
           "Pin the expected algorithm(s) explicitly in jwt.decode/jwt.verify; reject tokens with alg none.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html"], ["T1078", "T1190"], "CWE-347", ["jwt", "auth"]),
        _f("Server-side template injection (dynamic Jinja2 Template)", Severity.HIGH, "Code Injection",
           "Building a Jinja2 Template from user input executes template code ({{7*7}} proves SSTI, often RCE).",
           "Render templates by name with explicit context; never feed user input into Template().",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html"], ["T1059", "T1190"], "CWE-94", ["ssti", "rce"]),
        _f("os.popen shell execution", Severity.HIGH, "Command Injection",
           "os.popen runs a shell command; concatenated input enables command injection.",
           "Use subprocess.run with an argument list and shell=False.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html"], ["T1059"], "CWE-78", ["command-injection"]),
        _f("Django raw SQL query", Severity.CRITICAL, "SQL Injection",
           ".raw() with formatted SQL passes user data into the query text.",
           "Use the ORM or .raw() with %s params passed separately.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html"], ["T1190", "T1565"], "CWE-89", ["sqli"]),
    ],
    ".js": [
        ...
        _f("Open redirect via res.redirect(user input)", Severity.MEDIUM, "Open Redirect",
           "The redirect destination comes from the request, enabling phishing links on your domain.",
           "Map the parameter to a server-side allow-list of destinations.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html"], ["T1190"], "CWE-601", ["redirect"]),
        _f("Prototype pollution sink (merge/assign with request data)", Severity.HIGH, "Prototype Pollution",
           "Deep-merging request data into objects can pollute Object.prototype, leading to auth bypass or RCE in downstream libraries.",
           "Reject __proto__/constructor keys (validated schemas) and use pollution-safe merge utilities.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Prototype_Pollution_Prevention_Cheat_Sheet.html"], ["T1190", "T1059"], "CWE-1321", ["prototype-pollution"]),
        _f("Code execution via vm module", Severity.HIGH, "Code Injection",
           "Node's vm module is not a security boundary; running untrusted code in it is code execution.",
           "Use isolated worker processes with hardened flags, or a purpose-built sandbox (isolated-vm).",
           ["https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html"], ["T1059", "T1203"], "CWE-94", ["rce"]),
        _f("HTTP response header injection", Severity.MEDIUM, "Header Injection",
           "Setting headers (Location/Set-Cookie/Refresh) from user input allows response splitting/CR-LF injection.",
           "Strip CR/LF from any value placed into headers; encode per RFC.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html"], ["T1190"], "CWE-113", ["header-injection"]),
        _f("spawn with shell:true", Severity.MEDIUM, "Command Injection",
           "spawn(..., {shell:true}) routes arguments through the shell, enabling metacharacter injection.",
           "Remove shell:true and pass an argument array.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html"], ["T1059"], "CWE-78", ["command-injection"]),
        _f("JWT 'none' algorithm accepted (JS)", Severity.CRITICAL, "Authentication",
           "Accepting alg=none lets attackers forge valid-looking tokens.",
           "Pin algorithms in jwt.verify: algorithms: ['HS256'] (or your RS set).",
           ["https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html"], ["T1078", "T1190"], "CWE-347", ["jwt", "auth"]),
    ],
    ...
    ".java": [
        ...
        _f("JNDI lookup with user-controlled name", Severity.HIGH, "JNDI Injection",
           "A JNDI lookup name built from request data allows an attacker to point at a malicious LDAP/RMI endpoint and load attacker code (Log4Shell-class).",
           "Never pass user input to InitialContext.lookup; pin lookup names; run recent JDKs with trust URL restrictions.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html"], ["T1190", "T1203"], "CWE-74", ["jndi", "rce"]),
        _f("SpEL expression built from dynamic input", Severity.HIGH, "Code Injection",
           "Spring Expression Language evaluated on concatenated input is remote code execution.",
           "Evaluate only constant expressions; use SimpleEvaluationContext and never concatenate user data.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html"], ["T1059", "T1190"], "CWE-94", ["spel", "rce"]),
        _f("XPath built from concatenated input", Severity.HIGH, "XPath Injection",
           "XPath over concatenated input lets attackers alter the query and read unauthorized XML nodes.",
           "Use parameterized XPath (XPathVariableResolver) or validate input strictly.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html"], ["T1190"], "CWE-643", ["xpath"]),
        _f("File access with user-controlled path", Severity.HIGH, "Path Traversal",
           "A File stream is opened with a request parameter; traversal can read arbitrary files.",
           "Canonicalize and enforce a base directory allow-list.",
           ["https://owasp.org/www-community/attacks/Path_Traversal"], ["T1190", "T1083"], "CWE-22", ["traversal"]),
    ],
    ".php": [
        ...
        _f("File inclusion with user input (LFI/RFI)", Severity.CRITICAL, "File Inclusion",
           "include/require over $_GET/$_POST lets attackers execute local files (logs, uploads) or remote code.",
           "Map user input to a fixed allow-list of includable scripts; never include from request data.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html"], ["T1190", "T1059", "T1505.003"], "CWE-98", ["lfi", "rfi"]),
        _f("Open redirect via header('Location:')", Severity.MEDIUM, "Open Redirect",
           "The Location header is built from request data, enabling phishing redirects.",
           "Allow-list redirect destinations.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html"], ["T1190"], "CWE-601", ["redirect"]),
        _f("file_get_contents with user input (SSRF/LFI)", Severity.MEDIUM, "SSRF",
           "Fetching a URL or path from request data can reach internal services or local files.",
           "Validate scheme+host against an allow-list; block private ranges.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Server_Side_Request_Forgery_Prevention_Cheat_Sheet.html"], ["T1190"], "CWE-918", ["ssrf", "lfi"]),
        _f("preg_replace with /e modifier (code execution)", Severity.HIGH, "Code Injection",
           "The deprecated /e modifier evaluates the replacement as PHP code.",
           "Use preg_replace_callback; never /e (removed in PHP 7 — its presence implies ancient code).",
           ["https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html"], ["T1059"], "CWE-94", ["rce"]),
    ],
    ".go": [
        ...
        _f("Potential SQL string concatenation (fmt.Sprintf into Query)", Severity.CRITICAL, "SQL Injection",
           ...
           [R["sql"]], M_SQLI, "CWE-89", ["sqli"]),
        _f("Shell execution via sh -c", Severity.HIGH, "Command Injection",
           "exec.Command('sh', '-c', ...) routes the argument through the shell, enabling injection.",
           "Invoke the target binary directly with argument elements.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/OS_Command_Injection_Defense_Cheat_Sheet.html"], ["T1059"], "CWE-78", ["command-injection"]),
        _f("File path built from user input (Go)", Severity.MEDIUM, "Path Traversal",
           "filepath.Join with request data permits '../' traversal outside the intended root.",
           "filepath.Clean the result and verify it remains under the base directory.",
           ["https://owasp.org/www-community/attacks/Path_Traversal"], ["T1190", "T1083"], "CWE-22", ["traversal"]),
    ],
    ".cs": [
        ...
        _f("Potential SQL injection (string concatenation into SqlCommand)", Severity.CRITICAL, "SQL Injection",
           ...
           [R["sql"]], M_SQLI, "CWE-89", ["sqli"]),
        _f("Open redirect via Response.Redirect", Severity.MEDIUM, "Open Redirect",
           "The redirect target is taken from the request.",
           "Allow-list return URLs.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Unvalidated_Redirects_and_Forwards_Cheat_Sheet.html"], ["T1190"], "CWE-601", ["redirect"]),
        _f("Unescaped output via Html.Raw (XSS)", Severity.HIGH, "Cross-Site Scripting",
           "Html.Raw bypasses Razor encoding; dynamic values become XSS.",
           "Encode model output; only Html.Raw constant, reviewed markup.",
           ["https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html"], ["T1190"], "CWE-79", ["xss"]),
        _f("File path built from user input (C#)", Severity.HIGH, "Path Traversal",
           "Path.Combine with request data escapes the base path on absolute or traversal input.",
           "Use Path.GetFullPath and enforce a base-directory prefix check.",
           ["https://owasp.org/www-community/attacks/Path_Traversal"], ["T1190", "T1083"], "CWE-22", ["traversal"]),
    ],
}
...
# Kubernetes workload manifests (matched when a yaml has apiVersion+kind)
K8S_RULES = [
    (re.compile(r"(?m)^\s*privileged\s*:\s*true"),
     "Kubernetes container runs privileged", Severity.CRITICAL,
     "Privileged containers have root-equivalent host access (all capabilities, devices, host namespaces).",
     ["T1190", "T1078"], "CWE-250"),
    (re.compile(r"(?m)^\s*-?\s*hostPath\s*:"),
     "Kubernetes hostPath volume mounted", Severity.HIGH,
     "hostPath volumes expose host filesystem paths into the container — node-level escape.",
     ["T1005"], "CWE-732"),
    (re.compile(r"(?m)^\s*hostNetwork\s*:\s*true"),
     "Kubernetes pod uses hostNetwork", Severity.MEDIUM,
     "Pod shares the host network namespace — can sniff/impersonate node services.",
     ["T1005"], "CWE-732"),
    (re.compile(r"(?m)^\s*allowPrivilegeEscalation\s*:\s*true"),
     "Kubernetes allowPrivilegeEscalation: true", Severity.MEDIUM,
     "Setuid binaries inside the container can escalate to root.",
     ["T1078"], "CWE-250"),
    (re.compile(r"(?m)^\s*runAsUser\s*:\s*0\b"),
     "Kubernetes container forced to run as root (runAsUser: 0)", Severity.MEDIUM,
     "Run containers as a non-root UID and set runAsNonRoot: true.",
     ["T1078"], "CWE-250"),
    (re.compile(r"(?m)^\s*-\s*(SYS_ADMIN|NET_ADMIN|DAC_READ_SEARCH)\s*$"),
     "Dangerous Linux capability added to container", Severity.HIGH,
     "SYS_ADMIN/NET_ADMIN in a container approximate host root.",
     ["T1078"], "CWE-732"),
]

# docker-compose files (matched when a yaml declares services with images)
COMPOSE_RULES = [
    (re.compile(r"(?m)^\s*privileged\s*:\s*true"),
     "docker-compose service runs privileged", Severity.CRITICAL,
     "Privileged containers have root-equivalent host access.",
     ["T1190", "T1078"], "CWE-250"),
    (re.compile(r"(?m)^\s*(SYS_ADMIN|NET_ADMIN)\s*$"),
     "Dangerous capability granted to compose service", Severity.HIGH,
     "Remove cap_add entries; use the minimum capabilities required.",
     ["T1078"], "CWE-732"),
    (re.compile(r"(?m)^\s*pid\s*:\s*[\"']?host"),
     "Compose service shares the host PID namespace", Severity.MEDIUM,
     "Remove pid: host — it lets the container interact with all host processes.",
     ["T1005"], "CWE-732"),
    (re.compile(r"(?m)^\s*network_mode\s*:\s*[\"']?host"),
     "Compose service uses host networking", Severity.MEDIUM,
     "Bind specific ports instead of network_mode: host.",
     ["T1005"], "CWE-732"),
]
...
SINK_PATTERNS = {
    ...
    'Open redirect via unvalidated request parameter': 'redirect\\s*\\(\\s*request\\.(args|GET|form|values|full_path)',
    'Path traversal via user-controlled file path (send_file)': '(send_file|send_from_directory)\\s*\\(\\s*request\\.',
    'Path traversal via os.path.join with user input': 'os\\.path\\.join\\s*\\([^)]*request\\.(args|GET|form|values)',
    'JWT signature verification disabled': 'jwt\\.decode\\s*\\(.*(verify\\s*=\\s*False|verify_signature[\'\\"]?\\s*:\\s*False)',
    "JWT 'none' algorithm accepted": 'algorithms\\s*=\\s*\\[?\\s*[\'\\"]none[\'\\"]',
    'Server-side template injection (dynamic Jinja2 Template)': 'Template\\s*\\(\\s*(request|f[\'\\"])',
    'os.popen shell execution': '\\bos\\.popen\\s*\\(',
    'Django raw SQL query': '\\.raw\\s*\\(\\s*(f[\'\\"]|[\'\\"][^\'\\"]*%s[\'\\"]\\s*%)',
    'Open redirect via res.redirect(user input)': 'res\\.redirect\\s*\\(\\s*(req\\.(query|params|body)|[^)]*\\+\\s*req\\.)',
    'Prototype pollution sink (merge/assign with request data)': '(merge|defaultsDeep|assign)\\s*\\([^,)]*,\\s*(req\\.(body|query|params))',
    'Code execution via vm module': '\\bvm\\.(runInNewContext|runInThisContext|compileFunction|Script)\\s*\\(',
    'HTTP response header injection': 'setHeader\\s*\\(\\s*[\'\\"](Location|Set-Cookie|Refresh)[\'\\"]\\s*,[^)]*req\\.',
    'spawn with shell:true': 'spawn\\s*\\([^)]*shell\\s*:\\s*true',
    "JWT 'none' algorithm accepted (JS)": 'algorithms\\s*:\\s*\\[\\s*[\'\\"]none[\'\\"]',
    'JNDI lookup with user-controlled name': '\\.lookup\\s*\\(\\s*request\\.getParameter',
    'SpEL expression built from dynamic input': 'parseExpression\\s*\\([^)]*\\+',
    'XPath built from concatenated input': '(?i)xpath[^;]{0,60}\\+\\s*request\\.getParameter',
    'File access with user-controlled path': 'new\\s+File(Reader|InputStream|OutputStream)\\s*\\(\\s*request\\.getParameter',
    'File inclusion with user input (LFI/RFI)': '(include|require)(_once)?\\s*\\(?\\s*\\$_(GET|POST|REQUEST|COOKIE)',
    "Open redirect via header('Location:')": 'header\\s*\\(\\s*[\'\\"]Location:\\s*[\'\\"]?\\s*\\.\\s*\\$_(GET|POST|REQUEST)',
    'file_get_contents with user input (SSRF/LFI)': 'file_get_contents\\s*\\(\\s*\\$_(GET|POST|REQUEST)',
    'preg_replace with /e modifier (code execution)': 'preg_replace\\s*\\(\\s*[\'\\"][^\'\\"]*\\/e[\'\\"]',
    'Shell execution via sh -c': 'exec\\.Command(Context)?\\s*\\(\\s*[\'\\"](ba)?sh[\'\\"]\\s*,\\s*[\'\\"]-c[\'\\"]',
    'File path built from user input (Go)': 'filepath\\.Join\\s*\\([^)]*(r\\.FormValue|r\\.URL\\.Query)',
    'Open redirect via Response.Redirect': 'Response\\.Redirect\\s*\\(\\s*Request\\[',
    'Unescaped output via Html.Raw (XSS)': 'Html\\.Raw\\s*\\(\\s*[A-Za-z_(]',
    'File path built from user input (C#)': 'Path\\.Combine\\s*\\([^)]*Request\\[',
}
...
def scan(root: str, max_file_size: int = 2 * 1024 * 1024, progress=None, excludes: list | None = None) -> list:
    ...
    for i, (path, rel) in enumerate(files):
        ...
        if base.endswith(".yml") or base.endswith(".yaml"):
            ...
            elif "apiVersion:" in text and "kind:" in text:
                for rx, msg, sev, fix, mitre, cwe in K8S_RULES:
                    for j, line in enumerate(text.splitlines(), 1):
                        if rx.search(line):
                            findings.append(Finding(
                                scanner="sast", category="Kubernetes Manifest", title=msg,
                                severity=sev, target=rel, location=f"{rel}:{j}",
                                description=f"{msg} ({base}).", evidence=line.strip()[:160],
                                remediation=fix,
                                references=["https://kubernetes.io/docs/concepts/security/pod-security-standards/"],
                                mitre=mitre, cwe=cwe, tags=["k8s", "iac", "sast"],
                            ))
            elif ("services:" in text and "image:" in text) or "docker-compose" in base:
                for rx, msg, sev, fix, mitre, cwe in COMPOSE_RULES:
                    for j, line in enumerate(text.splitlines(), 1):
                        if rx.search(line):
                            findings.append(Finding(
                                scanner="sast", category="Container Configuration", title=msg,
                                severity=sev, target=rel, location=f"{rel}:{j}",
                                description=f"{msg} ({base}).", evidence=line.strip()[:160],
                                remediation=fix, references=[R["docker"]],
                                mitre=mitre, cwe=cwe, tags=["docker", "iac", "sast"],
                            ))
