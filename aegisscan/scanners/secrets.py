...
PATTERNS = [
    ...
    ("GitLab Personal Access Token",
     ...
     Severity.HIGH,
     "Revoke the token at GitLab -> Preferences -> Access Tokens and store it in a secret manager.",
     ["https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html"]),
    ("npm Access Token",
     re.compile(r"\bnpm_[A-Za-z0-9]{32,40}\b"),
     Severity.HIGH,
     "Revoke at npmjs.com -> Access Tokens; use granular tokens stored in CI secret storage.",
     ["https://docs.npmjs.com/managing-your-tokens"]),
    ("PyPI Upload Token",
     re.compile(r"\bpypi-AgEIcHlwaS5vcmc[A-Za-z0-9_\-]{30,}\b"),
     Severity.HIGH,
     "Revoke at pypi.org -> Account settings -> API tokens; prefer Trusted Publishing.",
     ["https://docs.pypi.org/trusted-publishers/"]),
    ("Telegram Bot API Token",
     re.compile(r"\b\d{8,10}:AA[A-Za-z0-9_\-]{30,35}\b"),
     Severity.MEDIUM,
     "Revoke via @BotFather (/revoke) and move the token to a secret store.",
     ["https://core.telegram.org/bots/tutorial#obtain-your-bot-token"]),
    ("Azure Storage Account Key",
     re.compile(r"AccountKey=[A-Za-z0-9+/=]{60,}"),
     Severity.HIGH,
     "Rotate the storage account key in the Azure portal; prefer SAS-scoped or Entra-ID auth.",
     ["https://learn.microsoft.com/en-us/azure/storage/common/storage-account-keys-manage"]),
    ("Facebook/Page Access Token",
     re.compile(r"\bEAACEdEose0cBA[A-Za-z0-9]{10,}\b"),
     Severity.MEDIUM,
     "Invalidate the token via the Graph API / App Dashboard and rotate.",
     ["https://developers.facebook.com/docs/graph-api/securing-tokens"]),
    ("Shopify Access Token",
     re.compile(r"\bshp(?:at|ca|pa)_[A-Fa-f0-9]{32}\b"),
     Severity.HIGH,
     "Revoke the app token in the Shopify partner dashboard; use scoped app tokens.",
     ["https://shopify.dev/docs/apps/auth/admin-app-access-tokens"]),
    ("Square Access Token",
     re.compile(r"\bEAAA[A-Za-z0-9_\-]{50,}\b|\bsq0atp-[0-9A-Za-z\-_]{20,}\b"),
     Severity.HIGH,
     "Revoke in the Square developer dashboard; keep tokens server-side only.",
     ["https://developer.squareup.com/docs/dashboard/credentials"]),
    ("X/Twitter Bearer Token",
     re.compile(r"\bAAAAAAAAA[A-Za-z0-9%]{20,}\b"),
     Severity.MEDIUM,
     "Regenerate the bearer token in the X developer portal.",
